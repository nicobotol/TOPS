# in this file we solve the PF with some values of line parameters (the ones loaded from the model), but we build the measurement matrix with a different value of line parameters. 
...
if __name__ == '__main__':
  ...
  # Measure Ibus
  mu_R = 0  # mean of the noise on R
  mu_X = 0  # mean of the noise on X
  error_R = 1e-3 # error on the measure of R
  error_X = 5e-3 # error on the measure of X
  sigma_R = (error_R/3) # standard deviation of the noise on R
  sigma_X = (error_X/3) # standard deviation of the noise on X

  # Modify the line parameters and build the new y_bus_matrix
  ps_modified = copy.deepcopy(ps) # copy the power system model
  for i in range(ps.n_lines):
    ps_modified.lines['Line'].data[i]['R'] = 1 + ps_modified.lines['Line'].data[i]['R'] + np.random.normal(loc=mu_R, scale=sigma_R, size=1) # modify the line parameters
    ps_modified.lines['Line'].data[i]['X'] = ps_modified.lines['Line'].data[i]['X'] + np.random.normal(loc=mu_X, scale=sigma_X, size=1) # modify the line parameters
  ...
  plt.tight_layout()
  plt.savefig('../assigments/system_estimation/results/combined_plots_topology_uncertainty.pdf')
  ...
  write_latex_table(case_names, MSE_errors, J_errors, 'topology_uncertainty')
